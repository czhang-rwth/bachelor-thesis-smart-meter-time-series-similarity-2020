...
def haushalt_einlesen():
    ...
    while i<=74:
        ...
        haushalt[i-1] = indexieren_htw_xlsx('datasets/HTW Berlin/1min_htwBerlin/Haushalt_' +str(i)+'.csv' ,
                                'datasets/HTW Berlin/1min_htwBerlin/Haushalt_'+str(i)+'.xlsx').resample('D').mean()
        i=i+1
    return haushalt
...
def plotten_alle(haushalt):
    ...
    i = 0
    while i < 74:
        plt.plot(haushalt[i].index, haushalt[i],color='blue', alpha=0.12)
        i=i+1

    # plt.plot(x, y, 's-',label=label)
    plt.xlabel('Zeit', fontsize=20)
    plt.ylabel('Wirkleistung[kW]', fontsize=20)
    plt.title('Wirkleistungen aller Haushalte mit Zeitaufloesung:1 Tag', fontsize=20, ha='center')
    plt.legend(loc='best', fontsize=17)  # fuer label
    plt.savefig('datasets/alle_Haushalte_day.png', bbox_inches='tight')
    plt.show()
...
haushalt_X = X.copy()
print(X)
X.to_excel('datasets/HTW Berlin/1min_htwBerlin/X_1day.xlsx')
...
#                   xlabel='Number of clusters', ylabel='average sillhoute',datei='datasets/Sillhoute_day.png')

kmeans=KMeans(n_clusters=3)
cluster_found=kmeans.fit_predict(X)
cluster_found_sr=pd.Series(cluster_found, name='cluster')

haushalt_X=haushalt_X.set_index(cluster_found_sr,append=True)
print(haushalt_X)
haushalt_X.to_excel('datasets/clustering_day_3clusters.xlsx')