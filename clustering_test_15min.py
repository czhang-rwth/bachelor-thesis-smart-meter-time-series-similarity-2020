...
def haushalt_einlesen():
    ...
    while i<=74:
        ...
        haushalt[i-1] = indexieren_htw_xlsx('datasets/HTW Berlin/1min_htwBerlin/Haushalt_' +str(i)+'.csv' ,
                                'datasets/HTW Berlin/1min_htwBerlin/Haushalt_'+str(i)+'.xlsx').resample('15T').mean()
        i=i+1
    return haushalt
...
def plotten_alle(haushalt):
    ...
    plt.title('Wirkleistungen aller Haushalte mit Aufloesung: 15min', fontsize=20, ha='center')
    plt.legend(loc='best', fontsize=17)  # fuer label
    plt.savefig('datasets/alle_Haushalte_15min.png', bbox_inches='tight')
    plt.show()
...
haushalt_X = X.copy()
print(X)
X.to_excel('datasets/HTW Berlin/1min_htwBerlin/X_15min.xlsx')
...
plotten_blau(n_cluster_list, sillhoute_scores, title='Average sillhoute score per number of clusters',
                   xlabel='Number of clusters', ylabel='average sillhoute',datei='datasets/Sillhoute_15min.png')
...
print(haushalt_X)
haushalt_X.to_excel('datasets/clustering_15min.xlsx')